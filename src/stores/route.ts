...
import { getRouteNames, transformRouteToMenu, getCacheRouteNames, filterRoutesByUserRole } from '@/helpers';
import { useTabStore, useAuthStore } from '@/stores';
...
export const useRouteStore = defineStore('route', {
  ...
  actions: {
    ...
    async dynamicInit() {
      ...
      const routes = filterRoutesByUserRole(dynamicRoutes, userInfo.role);
      // ! 这里有点隐藏的漏洞，虽然根据 meta.hidden 在菜单中隐藏了，但其实还是动态注入了路由表，直接通过 URL 还是可以访问到
      this.menus = transformRouteToMenu(routes); // 生成菜单
      routes.forEach((route) => {
        router.addRoute(route); // 动态注入路由组件
      });
      this.caches = getCacheRouteNames(routes); // 收集开启缓存的路由 name【保证组件 name 与路由 name 一致才生效】
    }
  }
});
